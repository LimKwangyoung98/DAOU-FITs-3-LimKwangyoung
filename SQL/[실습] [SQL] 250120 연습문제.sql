...
--모든 부서별 직무별 평균 급여와 해당 직무에서 가장 높은 급여를 계산하세요.
...
E.JOB_ID, ROUND(AVG(E.SALARY), 2) AVG_SALARY, MAX(E.SALARY) MAX_SALARY
FROM EMPLOYEES E
...
SELECT D.DEPARTMENT_NAME, E.JOB_ID, ROUND(AVG(E.SALARY), 2) AVG_SALARY, MAX(E.SALARY) MAX_SALARY
...
FROM DEPARTMENTS D,(
  SELECT DEPARTMENT_ID, CEIL(AVG(SALARY)) AVG_SALARY
  FROM EMPLOYEES
...
WHERE C.CUSTOMER_ID = L.CUSTOMER_ID
  AND L.BRANCH_ID IN (SELECT BRANCH_ID FROM BRANCHES WHERE LOCATION = 'Location 4')
  AND L.AMOUNT >= 50000;

--4. 대출 상태가 'PENDING'인 고객과 'REJECTED'인 고객의 교집합
--대출 상태가 'PENDING'인 고객과 'REJECTED'인 고객 중 두 상태 모두 해당되는 고객의 이름과 이메일, 전화번호를 조회하세요.
...
SELECT C.NAME "고객 이름", A.BALANCE "금액"
FROM CUSTOMERS C, ACCOUNTS A
WHERE C.CUSTOMER_ID = A.CUSTOMER_ID
  AND A.BALANCE >= 50000
UNION
SELECT C.NAME "고객 이름", L.AMOUNT "금액"
FROM CUSTOMERS C, LOANS L
WHERE C.CUSTOMER_ID = L.CUSTOMER_ID
